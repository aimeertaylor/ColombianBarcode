...
#' see how FSVC sids cluster with DE sids, generate clonal components from all
#' sids together. To compare 250 SNP based clonal components with WGS-based
#' clusters, generate clonal components from FSVC sids only. Where FSVC sids
#' belong to samples that didn't feature in Taylor et al. 2020 and DE sids
#' belong to data that did feature in Taylor et al. 2020. Because NA graph edges
#' are ambiguous (setting NAs to one augments connectivity, setting NAs to zero
#' decreases it), we remove samples with one or more NA relatedness estimates;
#' see Generate_sids_remv.R. Because the relatedness estimates for FSVC sample
#' ids are associated with more uncertainty than those for DE sids we consider
#' three different definitions of a clone: statistically indistinguishable from
#' one with UCI >= 1-eps (definition used in Taylor et al. 2020); statistically
#' indistinguishable from one but distinguishable from 0.75 with UCI >= 1-eps
#' and LCI > 0.75 (definition equivalent to that used in Taylor et al. 2020
#' because 0.75 approx. min(LCI of DE sid clones) - see below); and
#' statistically indistinguishable from one but distinguishable from 0.95 (this
#' definition is more stringent than the one used in Taylor et al. 2020 and so
#' breaks up many of the 46 clonal components listed in Taylor et al. 2020 - see
#' Compare_components.R)
#'
#' To-do, consider removing LCI > 0.95
################################################################################
rm(list = ls())
library(igraph) 
...
# Why are there some NA weights??
comparisons <- "all"
LCI_threshold <- 0
which(is.na(E(G_clonal)$weight)) 
