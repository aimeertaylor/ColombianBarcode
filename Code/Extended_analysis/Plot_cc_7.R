...
#====================================================================
# Inspection of individual subparts 
sids <- c(sids_nucleus, break2, break1) # Entire component
sids <- sids_nucleus # Just the nucleus
sid_list <- list(c(sids_nucleus, break2, break1), sids_nucleus, break1, break2)
#====================================================================

for(sid_i in 1:length(sid_list)) {
  
  sids <- sid_list[[sid_i]]
  
  pdf(file = sprintf("../../Plots/Plot_cc_7_%s.pdf",sid_i), 
      width = 8 + length(sids)/7, height = 8 + length(sids)/7)
  ...
  legend('bottomleft', pch = 16, bty = 'n', cex = 1, pt.cex = 2, 
  ...
  print(clique_count_per_component)
  dev.off()
}
