...
---
<!-- coloured haps by relatedness but I haven't had the time yet.  -->

A comment from Dan: 
Do you want to explicitly mention how 5 highly related haplotypes could connect Buenaventura and Tumaco via one event (a multiply infected person or mosquito)? You could mention COI distribution to emphasize low likelihood of a single event. 

To-do remind yourself of Choco Narino, find out MOI distribution, reformat akin to Seth's plot; Another way to ask fuzzy question would be how fuzzy does it need to be to capture all IBD = 1!
...
inter = (Result$site_comp == "Buenaventura_Tumaco")
intra = (Result$site_comp == "Buenaventura_Buenaventura") |  (Result$site_comp == "Tumaco_Tumaco")
ind_result = inter | intra
...
vertex.cols <- c("#FFFFFFFF", cols(no_repeat_haps)) # White then colours
...
The number of differently coloured repeat haplotypes might give a false impression of the number of independent pieces of information, however. A network of relatedness between the repeat haplotypes (white edge IBD = 0.001, darkest edge IBD = 0.83) was thus plotted (Figure \ref{fig: relatedness of repeat haps}). 
Of the haplotypes shared across Buenaventura and Tumaco, haplotypes 5, 7 and 12 are all highly related to one another, so might originate from a single parasite. On the contary, haplotypes 1 and 2 are not highly related to the above cluster nor one-another. Together these results suggest three travel events. 
...
plot(G_haps, 
     ...
     edge.color = gray(1-edge_attr(G_haps)$weight)) # Normalise 
