...
```{r, include = FALSE}
knitr::opts_chunk$set(echo=FALSE, warning=FALSE, message=FALSE, include=FALSE, cache=TRUE, cache.comments = FALSE, fig.pos = 'H', fig.width = 8, fig.height = 6, dev = 'png', dpi = 300)
```
...
load('../RData/Result.RData') 
load('../RData/Thresholds.RData') 
...
## Plots of $\widehat{r}$ and $\widehat{\text{IBS}}$

```{r IBD_hist, include = TRUE, fig.width=7, fig.cap= "\\label{fig: IBD distribution} Estimates of $\\widehat{r}$ for all pairwise comparisons of 325 samples."}
par(family = 'serif')
X = hist(Result$IBD, breaks = seq(0,1,0.025), plot = F)
...
```{r IBD_v_IBS, include = FALSE}
# create dataset with 1000 normally distributed points
df <- data.frame(x = Result$fract_sites_IBD, y = Result$IBS)
# create a ggplot2 scatterplot
p <- ggplot(df, aes(x, y)) + geom_point(colour = adjustcolor('black', alpha.f = 0.5)) + theme_classic() + labs(x = 'rhat', y = 'IBS') + geom_hline(yintercept = threshold_IBS, lty = 'dotted') + geom_vline(xintercept = threshold_IBD, lty = 'dotted')
...
par(family = 'serif')
hist(c(0,Result$IBS), breaks = 100, plot = TRUE, 
     panel.first = grid(), 
     col = cols[2], las = 2, xaxt = 'n', 
     xlim = c(0,1), xlab = '', 
     ylab = expression('Number of'~italic('P. falciparum')~'sample pairs'), main = '')
axis(side = 1, tick = F, line = -1)
title(xlab = expression(widehat(IBS)), line = 1)
abline(v = 1-h_constant, col = 'green', lwd = 2)
legend('topleft', legend = bquote(1-italic(h)==.(round(1-h_constant,2))), 
       fill = 'green', inset = 0.1)
```

## Plot of $\widehat{r}_m$ and $\widehat{\text{IBS}}_m$ against distance 
...
plot(y = Result$fract_sites_IBD, x = Result$geo_dist + rnorm(nrow(Result), 0, 5), 
     bty = 'n', pch = 21, bg = cols[1], cex = 1, 
     xlab = 'Inter-city distance (km)', ylab = expression(italic(widehat(r))))
abline(h = 0.5, lty = 'dotted')

plot(y = Result$IBS, x = Result$geo_dist + rnorm(nrow(Result), 0, 5), 
     bty = 'n', pch = 21, bg = cols[2], cex = 1, ylim = c(0,1),
     xlab = 'Inter-city distance (km)', ylab = expression(widehat(IBS)))
abline(h = 0.5, lty = 'dotted')
```

## Plot of $\widehat{r}_m$ and $\widehat{\text{IBS}}_m$ against time 
...
plot(y = Result$IBD, x = Result$time_dist, bty = 'n', 
     ...
     ylab = expression(italic(widehat(r))), main = '', pch = 20, 
     col = adjustcolor('black', alpha.f = 0.2))
plot(y = Result$IBS, x = Result$time_dist, bty = 'n', 
     xlab = 'Time difference (weeks)', ylim = c(0,1), 
     ylab = expression(widehat(IBS)), main = '', pch = 20, 
     col = adjustcolor('black', alpha.f = 0.2)) 
```
...
From Vladimir
\begin{itemize}
\item Guapi to Buenaventura: fast boat, 6-8 ppl, 4 hr; ship cargo, approx. 50 ppl, 12hr; 
Tumaco road is a bad road
\item no public transport between Guapi and Tumaco, only contract fast boat; 1 hr to El Charco (just south of Guapi) and 5hrs from El Charco to Tumaco. 
\item plantain produced in Tumaco is sold in Guapi (and probably Buenventura)
\item many goods that are received in Buenaventura (the major port of Colombia) are delivered to Tumaco
\end{itemize}

Via Diego (all the info is directly from the sites)
\begin{itemize}

\item By Ship (barco): price: around 80.000 – 100.000 COP (1 USD is  3050 COP)
\begin{itemize}
\item Tumaco – Buenaventura: about 17-18 hours.
\item Guapi – Buenaventura: about 10 – 12 hours
\end{itemize}

By Boat (lancha): price around 120.000 COP
\begin{itemize}
\item Buenaventura – Guapi: 4h
\item Guapi – Tumaco: 3 – 3,5 H
\item Buenaventura-Tumaco: No transportation. the sea rout is Buenaventura-Guapi and them Tumaco (and Vice-versa).A express transportation with good accommodation, food, Buenaventura-Guapi-Tumaco may cost up to 4.000.000 COP.
\end{itemize}

\item Road trip Between Guapi and the Panamerican freeway (the blue road in the map): No access at all.
\item The only inter-municipal road in Guapi es between Guapi (Cauca) and Chanzara (Narino), it is a rudimentary and dangerous way, about 2 hours. People do no use this road.
\end{itemize}
...
\begin{itemize}
\item Quidbo Tado: 1.5
\item Quidbo Buenaventura: 10 
\item Buenaventura Tado: 8.5
\iten Buenaventura Tumaco: 15
\item Tado Tumaco: 19.5
\item Quidbo Tumaco: 21 
\end{itemize}
...
X <- barplot(proportions_time_grouped[, ,'highIBD'], 
             ...
             cex.names = 0.75, ylim = c(0,max(CIs)), col = rainbow(no_site_comps))
...
X <- barplot(proportions_time_grouped[, ,'all'], las = 2, xaxt = 'n', 
             ...
             cex.names = 0.75, col = rainbow(no_site_comps))
axis(side = 1, at = X, labels = colnames(proportions_time_grouped))
```
...
#plot(pairwise_site_distance_all[inter[inter_sort$ix]]) # Check order by distance
...
segments(y0 = CIs[1,], y1 = CIs[2,], x0 = X, x1 = X)
axis(side = 1, at = X, las = 2, cex.axis = 0.5, tick = F, 
     labels = sapply(intra_inter, function(x)gsub('_', ' & ',x)))
legend('top',density = c(100,35), fill = cols[2], bty = 'n', 
       legend = c('within site', 'across sites'))
...
X = barplot(proportions_dist[intra_inter, 'IBD'], 
            ...
            cex.names = 0.5, ylim = c(0,max(CIs)))
segments(y0 = CIs[1,], y1 = CIs[2,], x0 = X, x1 = X)
legend('right',density = c(100,35), fill = cols[1], bty = 'n', 
       legend = c('within site', 'across sites'))
...
# Break down it to site comparsion contributions 
CIs <- rbind(proportions_dist[comparison_names,'IBD'],
             proportions_dist[comparison_names, 'IBD']) + deltaCIs_dist[,comparison_names,'IBD']
X <- barplot(proportions_dist_grouped[,comparison_names ,'highIBD'], las = 2, xlab = '', 
             ylab = bquote('Proportion of'~~italic(widehat(r))>.(round(threshold_IBD, 1))), 
             cex.names = 0.5, ylim = c(0,max(CIs)), col = rainbow(no_time_bins))
segments(y0 = CIs[1,], y1 = CIs[2,], x0 = X, x1 = X)
legend('topleft', fill = rainbow(no_time_bins), bty = 'n',legend = time_bins)

# Grouped version 
X <- barplot(proportions_dist_grouped[,comparison_names,'all'], las = 2, xlab = '', 
             ylab = expression('Proportion of'~italic(widehat(r))), 
...
# Break down it to site comparsion contributions 
CIs <- rbind(proportions_dist[intra_inter,'IBD'], proportions_dist[intra_inter, 'IBD']) + deltaCIs_dist[,intra_inter,'IBD']
X <- barplot(proportions_dist_grouped[,intra_inter ,'highIBD'], las = 2, xlab = '', 
             ylab = bquote('Proportion of'~~italic(widehat(r))>.(round(threshold_IBD, 1))), 
             cex.names = 0.5, ylim = c(0,max(CIs)), col = rainbow(no_time_bins))
segments(y0 = CIs[1,], y1 = CIs[2,], x0 = X, x1 = X)
