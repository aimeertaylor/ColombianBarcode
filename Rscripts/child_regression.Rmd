...
# Hack (add time dist)
load('~/Dropbox/IBD_IBS/RData/Results.RData') 
Result = Results$Colombia
head(Result)
...
geo_dist_info$pairwise_site_time_all

X1 = X2 = pairwise_site_distance[,4]
names(X1) = apply(pairwise_site_distance[,1:2], 1, paste, collapse = '_')
names(X2) = apply(pairwise_site_distance[,2:1], 1, paste, collapse = '_')
travel_times_all = c(X1,X2)
ind = Result$site_comp %in% names(travel_times_all)
Result$travel_time = 0
Result$travel_time[ind] = travel_times_all[Result$site_comp[ind]]
unique(Result$travel_time)
```
...
```{r}
# Adding travel time
M3_pls_time <- '~ geo_dist + time_dist + travel_time + Tumaco + Guapi + Buenaventura + Quibdo + Tado'
M3_sub_time <- '~ travel_time + time_dist + Tumaco + Guapi + Buenaventura + Quibdo + Tado'
LM3_pls_time_IBD <- lm(sprintf('IBD %s', M3_pls_time), data = Result,  na.action = na.exclude)
LM3_sub_time_IBD <- lm(sprintf('IBD %s', M3_sub_time), data = Result,  na.action = na.exclude)
print(summary(LM3_pls_time_IBD))# All highly signif with signs as expected
print(summary(LM3_sub_time_IBD))# All highly signif with signs as expected
```
